...
export class PushNotificationGateway {
  ...
  handleSendMessage(@MessageBody() data: { from: string; to: string; message: string }) {
    ...
    if (socketId) {
      ...
      // Supprimer le message après 10 secondes (message éphémère)
      setTimeout(() => {
        this.server.to(socketId).emit('delete-message', { from: data.from });
        console.log(`Message de ${data.from} supprimé.`);
      }, 10000);
    } else {
